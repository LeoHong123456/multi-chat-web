    ...
    private void generateMenu(String entityName) {
        ...
        if (entity != null) {
            ...
            if (result == 1) {
                ...
                Integer[] funcList = new Integer[]{1, 5, 10, 15, 20, 25};
                for (Integer item : funcList) {
                    ...
                    if (item.equals(1)) {
                        // 查询
                        menu.setName(String.format("查询%s", moduleTitle));
                        menu.setUrl(String.format("/%s/list", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:list", entityName.toLowerCase()));
                    } else if (item.equals(5)) {
                        // 添加
                        menu.setName(String.format("添加%s", moduleTitle));
                        menu.setUrl(String.format("/%s/add", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:add", entityName.toLowerCase()));
                    } else if (item.equals(10)) {
                        // 修改
                        menu.setName(String.format("修改%s", moduleTitle));
                        menu.setUrl(String.format("/%s/update", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:update", entityName.toLowerCase()));
                    } else if (item.equals(15)) {
                        // 删除
                        menu.setName(String.format("删除%s", moduleTitle));
                        menu.setUrl(String.format("/%s/delete", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:delete", entityName.toLowerCase()));
                    } else if (item.equals(20)) {
                        // 设置状态
                        menu.setName("设置状态");
                        menu.setUrl(String.format("/%s/setStatus", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:status", entityName.toLowerCase()));
                    } else if (item.equals(25)) {
                        // 批量删除
                        menu.setName("批量删除");
                        menu.setUrl(String.format("/%s/batchDelete", entityName.toLowerCase()));
                        menu.setPermission(String.format("sys:%s:dall", entityName.toLowerCase()));
                    }
                    // 创建权限节点
